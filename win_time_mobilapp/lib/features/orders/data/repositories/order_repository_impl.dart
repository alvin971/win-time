...
class OrderRepositoryImpl implements OrderRepository {
  ...
  OrderRepositoryImpl(this.remoteDataSource, this.websocketService);
  ...
  Stream<OrderEntity> watchOrder(String orderId) {
    // Commencer à suivre cette commande via WebSocket
    websocketService.watchOrder(orderId);

    // Écouter les mises à jour depuis le WebSocket
    return websocketService.orderUpdates
        .where((update) => update['order_id'] == orderId || update['id'] == orderId)
        .asyncMap((update) async {
      // Récupérer la commande complète depuis l'API pour avoir toutes les données
      try {
        final orderModel = await remoteDataSource.getOrderById(orderId);
        return orderModel.toEntity();
      } catch (e) {
        // En cas d'erreur, retourner null ou gérer autrement
